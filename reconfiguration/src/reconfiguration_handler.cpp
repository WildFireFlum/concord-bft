...
BftReconfigurationHandler::BftReconfigurationHandler() {
  ...
  verifier_ = Factory::getVerifier(key_str, signingAlgorithm, KeyFormat::PemFormat);
  LOG_INFO(GL,
           "Initialized BftReconfigurationHandler with operator details: " << KVLOG(verifier_->getPubKey(),
                                                                                    signingAlgorithm));
}
